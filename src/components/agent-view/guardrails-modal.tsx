...
export function GuardrailsModal({ open, agentName, agentId, sessionKey, onOpenChange }: GuardrailsModalProps) {
  ...
  useEffect(() => {
    ...
    const parsed = normalizedId
      ? parseGuardrails(window.localStorage.getItem(`mgtsuite:guardrails:${normalizedId}`))
      : DEFAULT_CONFIG
  ...
  function handleSave() {
    ...
    window.localStorage.setItem(`mgtsuite:guardrails:${normalizedId}`, JSON.stringify(config))
