...
function deriveStatus(
  ...
  if (session.enabled === false || statusText.includes('paused'))
    return 'paused'
...
export function SquadStatusWidget({ editMode }: SquadStatusWidgetProps) {
  ...
  const agents = useMemo(() => {
    const allSessions = Array.isArray(sessionsQuery.data)
      ? sessionsQuery.data
      : []
    const now = Date.now()
  ...
  return (
    <WidgetShell
      ...
      editMode={editMode}
      action={
        <span className="inline-flex items-center rounded-full border border-neutral-200 bg-neutral-100/80 px-2 py-0.5 text-[11px] font-medium text-neutral-600 tabular-nums dark:border-neutral-700 dark:bg-neutral-800 dark:text-neutral-300">
          {agents.length}
        </span>
      }
      ...
      {sessionsQuery.isLoading && agents.length === 0 ? (
        ...
        <div className="space-y-1.5">
          {visibleAgents.map((agent) => (
            <div
              key={agent.id}
              className="flex items-center gap-2 rounded-lg px-2 py-1.5 transition-colors hover:bg-neutral-50 dark:hover:bg-white/10 dark:hover:bg-neutral-800/50"
            >
              <span
                className={cn(
              ...
              <p className="hidden max-w-[120px] truncate text-[11px] italic text-neutral-500 dark:text-neutral-400 sm:block">
                {agent.taskPreview || '—'}
              </p>
              <span className="shrink-0 text-[11px] text-primary-500 dark:text-neutral-400 tabular-nums">
                {agent.timeAgo}
              </span>
              {agent.tokens > 0 && (
                <span className="shrink-0 text-[10px] tabular-nums text-primary-500 dark:text-neutral-400">
                  {formatTokenCompact(agent.tokens)}
                </span>
              )}
              <span className="shrink-0 rounded-full bg-neutral-100 px-1.5 py-0.5 text-[10px] font-medium text-neutral-600 dark:bg-neutral-800 dark:text-neutral-300">
                {agent.modelShort}
              </span>
            </div>
          ))}
          {agents.length > 6 ? (
            <button
              ...
              className="mt-1 inline-flex text-[11px] font-medium text-neutral-500 dark:text-neutral-400 transition-colors hover:text-neutral-700 dark:hover:text-neutral-200"
            >
              View all →
            </button>
          ) : null}
