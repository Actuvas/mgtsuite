...
const TAP_DEBUG_STORAGE_KEY = 'mgtsuite:tap-debug'
const TAP_DEBUG_EVENT = 'mgtsuite:tap-debug-change'
...
export function useTapDebug(
  ...
  useEffect(() => {
    ...
    const applyEnabled = (next: boolean) => {
      enabled = next
      debugWindow.__MGTSUITE_TAP_DEBUG__ = next
      try {
